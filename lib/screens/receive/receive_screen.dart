...
class _ReceiveScreenState extends State<ReceiveScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      if (_tabController.index == 1 && lightningInvoice.isEmpty && !_isLoadingLn) {
  ...
  Future<void> _generateLnInvoice(int sats) async {
    ...
    try {
      ...
      String invoice;
      if (sats == 0) {
        invoice = await ldk.createZeroSatInvoice(description: "Sagali Flexible Payment");
      } else {
        invoice = await ldk.createInvoice(amountSat: sats, description: "Sagali Fixed Payment");
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("LDK Error: $e"), backgroundColor: Colors.redAccent),
        );
      }
    }
  }
                      ...
                      children: [
                        ...
                        Text(
                          _tabController.index == 0 
                              ...
                              : (_isFixedAmount ? "Fixed Lightning Invoice" : "Lightning Invoice"),
  ...
  // ... (rest of the helper methods _buildQrSection, _buildAddressText, _showAmountSheet)
  // ... Keep them the same as your current version
  ...
  void _showAmountSheet() {
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return Container(
          // padding handles the keyboard height dynamically
          padding: EdgeInsets.only(
            bottom: MediaQuery.of(context).viewInsets.bottom,
          ),
          // This ensures the sheet doesn't try to be taller than the available space
          child: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 24.0, vertical: 32.0),
              child: Column(
                mainAxisSize: MainAxisSize.min, // Vital: Shrinks the column to fit content
